...
- [lib/gh.ts](lib/gh.ts) — shells out to `gh auth status` / `gh auth token -u <account>` to discover multi-account setups and fetch tokens on demand. Tokens are **not** persisted by Glass; the source of truth is the `gh` CLI keychain.
